var table_templates = {
    ...
    th : '<th><%= header %></th>',
    td : '<td><%= cell %></td>',
    project: '<tr id="row_<%= id %>" class="<%= even %> rowsector_<%= sector_code %>" project-id="<%= id %>"><td><%= id %></td><td><%= project_name %></td><td>$<%= totalamt %> million</td><td><%= mjsector1 %></td><td><%= boardapprovaldate %></td></tr>'
};
