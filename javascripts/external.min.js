  ...
  F1.Poller.prototype = {
    ...
    */ 
    request: function(){
      ...
      if (req_ids.length){
        jq.ajax({url:this.poll_url + '?ids='+req_ids.join(',') + '&callback=?',
          dataType: 'json', 
          jsonpCallback: 'pollJSON',
  ...
  F1.Maker.Map = function (options) {
    ...
    if (this.options.view == 'javascript') {
      ...
      if (this.options.embed) {
        var url = this.add_auth(F1.host+'/maps/'+this.options.map_id+'cb.json?',{}, false);
    ...
    */
    ...
    load_layer: function(guid) {
      ...
      if ( jq.inArray(guid, self.layer_queue) > -1 )
      {
        ...
        self._layer_count[finder_id] = self._layer_count[finder_id] === undefined ? 0 : (self._layer_count[finder_id] += 1)
        var url = self.add_auth(F1.host + "/overlays/" + finder_id + "_" + self._layer_count[finder_id] + "cb.json?include_features=0&include_attributes=1", true);
        ...
        jq.ajax({url: url, dataType: 'json',  jsonpCallback: 'parseLayerJson' + finder_id + "_" + self._layer_count[finder_id], success: function(data) {
      ...
      */
      ...
      this.load_layer = function(guid) {
        ...
        if (self.layer_queue.indexOf(guid) > -1)
        {
          ...
          jq.ajax({ url: url, dataType: 'json', callback: 'parseLayerJson', success: function(data) {
            jq.extend(data, self._layers[guid]);
            self._layers[guid] = data;
